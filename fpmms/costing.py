...
# from mech.tools.prediction_request_embedding import prediction_sentence_embedding
...
class TokenCostCalculator:
    ...
    def token_to_cost(tokens: int, model: str, tokens_type: str) -> float:
        if model in TokenCostCalculator.TOKEN_PRICES and tokens_type in ['input', 'output']:
...
class CostCounter:
    ...
    def __init__(self, tool: str, prediciton_prompt: str, final_response: Optional[str] = None):

        # Input variables
        self.tool = tool
        self.prediciton_prompt = prediciton_prompt # assumes additional information is already added to the prompt
        self.prediciton_response = final_response # usually this is None; we use 100 tokens as default
    ...
    def get_cost(self, input: str, output: Optional[str] = None, model: str = 'gpt-3.5-turbo') -> float:
        ...
        if output:
            ...
            output_cost = TokenCostCalculator.token_to_cost(output_tokens, model, 'output')
        else:
            self.total_output_tokens += self.DEFAULT_OUTPUT_TOKENS
            output_cost = TokenCostCalculator.token_to_cost(self.DEFAULT_OUTPUT_TOKENS, model, 'output') # 200 tokens if response is not provided
    ...
    def run(self) -> float:
        ...
        if 'offline' in self.tool:
            if self.tool in ['prediction-offline', 'claude-prediction-offline']:
                ...
                self.prediciton_response_cost = self.get_cost(final_prompt, self.prediciton_response, config['model'])
            ...
            self.search_query_cost = self.get_cost(search_query_prompt, None, config['model'])

            # Final LLM call cost including additional information from Google search
            final_prompt = self.prediciton_prompt 
            self.prediciton_response_cost = self.get_cost(final_prompt, self.prediciton_response, config['model'])

        self.final_cost = self.prediciton_response_cost + self.google_search_cost + self.search_query_cost + self.sme_prompt_cost
