...
def eth_price_usd_q(bundle_id: int, block: Optional[int] = None) -> str:
...
def block_from_timestamp_q(timestamp: int) -> str:
...
def top_n_pairs_q(top_n: int) -> str:
...
def pairs_q(block: int, top_n_ids: List[str]) -> str:
